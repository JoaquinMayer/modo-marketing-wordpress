 ...
 * @return array
 */
function Modo_Marketing_body_classes($classes)
{
...
add_filter('body_class', 'Modo_Marketing_body_classes');
 ...
 * @return array
 */
function Modo_Marketing_post_classes($classes)
{
	$classes[] = 'entry';

	return $classes;
}
add_filter('post_class', 'Modo_Marketing_post_classes', 10, 3);
 ...
 * @return void
 */
function Modo_Marketing_pingback_header()
{
	if (is_singular() && pings_open()) {
		echo '<link rel="pingback" href="', esc_url(get_bloginfo('pingback_url')), '">';
	}
}
add_action('wp_head', 'Modo_Marketing_pingback_header');
 ...
 * @return void
 */
function Modo_Marketing_supports_js()
{
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action('wp_footer', 'Modo_Marketing_supports_js');
 ...
 * @return array
 */
function Modo_Marketing_comment_form_defaults($defaults)
{
...
add_filter('comment_form_defaults', 'Modo_Marketing_comment_form_defaults');
 ...
 * @return bool
 */
function Modo_Marketing_can_show_post_thumbnail()
{
	/**
 ...
 * @return int
 */
function Modo_Marketing_get_avatar_size()
{
	return 60;
}
 ...
 * @since Modo Marketing 1.0
 */
function Modo_Marketing_continue_reading_text()
{
 ...
 * @since Modo Marketing 1.0
 */
function Modo_Marketing_continue_reading_link_excerpt()
{
	if (!is_admin()) {
		return '&hellip; <a class="more-link" href="' . esc_url(get_permalink()) . '">' . Modo_Marketing_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter('excerpt_more', 'Modo_Marketing_continue_reading_link_excerpt');
 ...
 * @since Modo Marketing 1.0
 */
function Modo_Marketing_continue_reading_link()
{
	if (!is_admin()) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url(get_permalink()) . '#more-' . esc_attr(get_the_ID()) . '">' . Modo_Marketing_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter('the_content_more_link', 'Modo_Marketing_continue_reading_link');

if (!function_exists('Modo_Marketing_post_title')) {
	/**
 ...
 * @return string
 */
function Modo_Marketing_get_icon_svg($group, $icon, $size = 24)
{
	return Modo_Marketing_SVG_Icons::get_svg($group, $icon, $size);
}
 ...
 * @return string
 */
function Modo_Marketing_change_calendar_nav_arrows($calendar_output)
{
	$calendar_output = str_replace('&laquo; ', is_rtl() ? Modo_Marketing_get_icon_svg('ui', 'arrow_right') : Modo_Marketing_get_icon_svg('ui', 'arrow_left'), $calendar_output);
	$calendar_output = str_replace(' &raquo;', is_rtl() ? Modo_Marketing_get_icon_svg('ui', 'arrow_left') : Modo_Marketing_get_icon_svg('ui', 'arrow_right'), $calendar_output);
	return $calendar_output;
}
add_filter('get_calendar', 'Modo_Marketing_change_calendar_nav_arrows');
 ...
 * @return string
 */
function Modo_Marketing_get_non_latin_css($type = 'front-end')
{
	...
	$font_family = apply_filters(
		'Modo_Marketing_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'Modo_Marketing_get_localized_font_family_elements',
		array(
	...
	if (!function_exists('Modo_Marketing_generate_css')) {
	...
	return Modo_Marketing_generate_css( // @phpstan-ignore-line.
 ...
 */
function Modo_Marketing_print_first_instance_of_block($block_name, $content = null, $instances = 1)
{
	$instances_count = 0;
	$blocks_content  = '';
 ...
 *
 * @since Modo Marketing 1.0
 * @since Modo Marketing 1.4 Corrected parameter name for `$output`,
 ...
 */
function Modo_Marketing_password_form($output, $post = 0)
{
	$post   = get_post($post);
...
add_filter('the_password_form', 'Modo_Marketing_password_form', 10, 2);
 ...
 * @return array
 */
function Modo_Marketing_get_attachment_image_attributes($attr, $attachment, $size)
{
...
add_filter('wp_get_attachment_image_attributes', 'Modo_Marketing_get_attachment_image_attributes', 10, 3);
